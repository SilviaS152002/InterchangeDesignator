...
namespace NS15
{
    namespace InterchangeDesignation
    {
        ...
        public static class OpsControllerPatches
        {
            ...
            [HarmonyPatch("WaybillCarToInterchange"), HarmonyTranspiler, HarmonyDebug]
            public static IEnumerable<CodeInstruction> WaybillCarToInterchangeTranspiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
            {
                List<CodeInstruction> codes = instructions.ToList();
                MethodInfo mi_UpdatePayment = AccessTools.Method(typeof(OpsControllerPatches), "UpdatePayment");
                ...
                for (int i = 0; i < codes.Count; i++)
                {
                    if (i + 13 < codes.Count &&
                        ...
                        codes[i + 10].opcode == OpCodes.Stloc_1)
                    {
                        codes[i + 9].labels.Add(paymentZeroLabel);
                        codes[i + 10].labels.Add(paymentJumpLabel);
            ...
            public static int UpdatePayment(int payment, OpsCarPosition position)
            {
                // Debug.Log($"Payment pre-process: {payment}");
                Industry industry = OpsController.Shared.AllIndustries.First(ind => ind.Components.Any(comp => comp.Identifier == position.Identifier));
        ...
        public static class IndustryDetailBuilderPatch_AddContractSection
        {
            ...
            public static void Postfix(Industry ____industry, UIPanelBuilder builder)
            {
                ...
                builder.AddSection("Specify Interchange", delegate (UIPanelBuilder icBuilder)
                {
                    ...
                    if (interchangeId != 0)
                    {
                        string payDiffString = Main.settings.PayMultiplier > 1.01 ? $"{(int)(Main.settings.PayMultiplier * 100) - 100}% more" : Main.settings.PayMultiplier < 0.99 ? $"{100 - (int)(Main.settings.PayMultiplier * 100f + 0.5f)}% less" : "no different";
                ...
                bool NeedsInterchange(IndustryComponent c)
                {
                    ...
                    bool need = false;
                    if (c is IndustryLoaderBase clb) { need |= clb.orderEmpties; }
                    if (c is IndustryLoader cl) { need |= cl.orderAwayLoaded; }
                    if (c is IndustryUnloader cu) { need |= cu.orderAwayEmpties || cu.orderLoads; }
                    return need;
                }
            }
        }
